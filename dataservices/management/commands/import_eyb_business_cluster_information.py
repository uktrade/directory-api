...
from dataservices.management.commands.helpers import ingest_data, BaseDataWorkspaceIngestionCommand
...
def get_sic_codes_dit_sector_mapping():
    sql = """
        SELECT
...
class Command(BaseCommand, S3DownloadMixin, BaseDataWorkspaceIngestionCommand):
    ...
    def handle(self, *args, **options):
        ...
        data = get_sic_codes_dit_sector_mapping()
        save_uk_business_employee_counts_data(data)

        delete_temp_tables(['dataservices_tmp_eybbusinessclusterinformation', 'dataservices_tmp_sic_codes_dit_sector_mapping',])
        table_name = 'dataservices_tmp_sic_codes_dit_sector_mapping'
