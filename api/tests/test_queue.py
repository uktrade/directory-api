...
class ExampleQueueService(queue.SingletonMixin, queue.QueueService):
...
class TestService(MockBoto):
    ...
    def test_get_or_create_queue_non_existent_exception(self):
        """ Test queue.Service.get_or_create_queue

        NonExistentQueue exception should be handled by
        calling queue.Service.create_queue
        """
        ...
        queue_service._sqs.get_queue_by_name.side_effect = ClientError(
            ...
            error_response={
                'Error': {
                    'Code': queue.AwsErrorCodes.SQS_NON_EXISTENT_QUEUE
                },
            }
        )
    ...
    def test_receive_reinitialise_sqs_on_signature_not_match(self):
        """ Test queue.Service.receive reinitialises SQS connection on
        ...
        queue_service._sqs.get_queue_by_name.side_effect = ClientError(
            ...
            error_response={
                'Error': {
                    'Code': queue.AwsErrorCodes.SIGNATURE_DOES_NOT_MATCH
                },
            }
        )
        queue_service.get_or_create_queue(name='non-existent')
